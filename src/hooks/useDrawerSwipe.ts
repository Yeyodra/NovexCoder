...
export function useDrawerSwipe(options: DrawerSwipeOptions = {}) {
  ...
  const handleTouchMove = React.useCallback((e: React.TouchEvent) => {
    ...
    if (isHorizontalSwipeRef.current === true) {
      ...
      const leftDrawerWidthPx = drawer.leftDrawerWidth.current;
      const rightDrawerWidthPx = drawer.rightDrawerWidth.current;

      // Determine which drawer is being dragged
      ...
      if (isDraggingDrawerRef.current === 'right') {
        if (drawer.rightDrawerOpen) {
          ...
          drawer.rightDrawerX.set(rightDrawerWidthPx * progress);
        } else {
          const progress = Math.max(0, Math.min(1, -deltaX / rightDrawerWidthPx));
  ...
  const handleTouchEnd = React.useCallback((e: React.TouchEvent) => {
    ...
    if (isDraggingDrawerRef.current === 'left') {
      ...
      if (shouldComplete) {
        ...
        animate(drawer.leftDrawerX, targetX, springConfig);
        if (isOpen) {
    ...
    if (isDraggingDrawerRef.current === 'right') {
      ...
      if (shouldComplete) {
        ...
        animate(drawer.rightDrawerX, targetX, springConfig);
        if (isOpen) {
